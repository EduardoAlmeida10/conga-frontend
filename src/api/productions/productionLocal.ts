...
export async function updateLocalProduction(id: string, dto: UpdateLocalProductionDto): Promise<LocalProductionApiData> {
    const response = await api.put(`${BASE_URL}/${id}`, dto);
    return response.data;
}

/** @DELETE /productions/local/:id */
export async function deleteLocalProduction(id: string): Promise<{ message: string }> {
